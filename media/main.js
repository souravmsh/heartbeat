    ...
    function renderCalendar() {
        ...
        if (calContainer) {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                const holidayMatch = state.holidays.find(h => h.startDate <= dateStr && (!h.endDate || h.endDate >= dateStr));
    ...
    function updateSalahCountdown(now) {
        ...
        if (nextPrayer && nextTimeObj) {
            ...
            if (diffMs > 0 && diffMs <= 1000) {
                ...
                // Trigger native as well
                vscode.postMessage({ type: 'showNativeNotification', value: msg });
            }
